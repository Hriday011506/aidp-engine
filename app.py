...
def generate():
    ...
    suggested = float(market * (1 + np.clip((pred-300)/3000,-.08,.08))) if isinstance(market,(int,float)) else None
...
def health(p):
    if not p: return 0
    score = (np.clip(65+(p["trend"]-50)*.35,50,94) + np.clip(92-max(0,(p["pred"]-1200)/40),60,94) + (80 if p["market"] is not None else 68) + np.clip(92-abs(p["temp"]-27)*1.4,60,94)) / 4
...
def inputs():
    ...
    if changed:
        ...
        st.info("Inputs changed. Generate a new AI decision.")
    x,y=st.columns(2)
    with x:
        if st.button("Refresh market price",use_container_width=True,key="price_button"):
            st.info("Live market lookup is optional in this build. Forecasting and simulation work without it.")
    with y:
...
def simulator():
    ...
    x=pd.DataFrame({"holiday":[p["holiday"]],"temp":[p["temp"]+temp_change],"trend":[p["trend"]]});d=float(max(0,MODEL.predict(x)[0]))*(1+demand_change/100)*(1+promo/100);base_price=p["market"] if isinstance(p["market"],(int,float)) else 50;price=base_price*(1+price_change/100);revenue=d*price;stock=int(np.ceil(d*1.1));risk="Low" if stock<1800 else "Medium" if stock<3000 else "High"
...
def dashboard():
    ...
    vals=[("Forecasted demand",f"{int(p['pred']):,} units" if p else "—","Selected month"),("Recommended stock",f"{int(p['stock']):,} units" if p else "—","10% buffer"),("Market reference",f"₹{p['market']:,.2f}" if p and isinstance(p.get('market'),(int,float)) else "—","Optional signal"),("Retail health",f"{health(p)}/100" if p else "—","AI health score")]
    ...
    with b:
        ...
        st.markdown("</div>",unsafe_allow_html=True)
    if not p:st.info("Open Product Analysis and generate an AI decision.");return
...
def product_page():
    header("PRODUCT ANALYSIS","Analyze a product with <span>real market context.</span>","Select a product and location, then generate an explainable AI decision.");inputs();p=st.session_state.prediction
    if matches(p):st.markdown(f"<div class='card'><div class='eyebrow'>LATEST RESULT</div><h3>{html.escape(p['product'])} · {html.escape(p['city'])} · {p['month']}</h3><p class='small'>Forecast demand: <b>{int(p['pred']):,}</b> units · Weather: <b>{p['temp']:.1f} °C</b> · Holiday days: <b>{p['holiday']}</b> · Confidence: <b>{p['confidence']}%</b> · Source: <b>{p['source']}</b></p></div>",unsafe_allow_html=True)


def forecast_page():
    header("DEMAND FORECASTING","See demand change <span>across the year.</span>","Each month gets its own weather value; no repeated 26.1 °C placeholder.");p=st.session_state.prediction
...
def copilot_page():
    ...
    if st.button("Analyze question →",use_container_width=True,key="ask"):
        ...
        if not matches(p):st.info("Generate a decision first.")
            ...
            elif "price" in q:a="A market reference is optional in this presentation build; use the Simulator to test price changes."
...
def settings_page():
    header("SETTINGS","Configure your <span>workspace.</span>","This presentation build intentionally uses session storage so MongoDB authentication cannot block the demo.");u=st.session_state.user or {};st.text_input("Email",value=u.get("email",""),disabled=True);st.text_input("GST Number",value=u.get("gst",""),disabled=True);st.info("MongoDB and Brevo are not required for this version.")
...
if st.session_state.page=="welcome":
    header("OPTIRETAIL AI","Turn market data into <span>smarter decisions.</span>","Forecast demand, simulate scenarios, detect risks and explain AI recommendations.");a,b,c,d=st.columns(4)
    ...
    if st.session_state.page=="dashboard":header("DASHBOARD","Turn market data into <span>smarter decisions.</span>","AI Action Center, health score, explainable forecast and simulator.");dashboard()
    ...
    elif st.session_state.page=="market":header("MARKET INSIGHTS","Live market <span>reference.</span>","Optional market intelligence. The app works without it.");st.info("Market lookup is optional in this presentation build.")
        ...
        for p in st.session_state.saved[:10]:st.markdown(f"<div class='card'><h3>{html.escape(p['product'])} · {p['city']} · {p['month']}</h3><p class='small'>Demand {int(p['pred']):,} · Stock {int(p['stock']):,} · Confidence {p['confidence']}%</p></div>",unsafe_allow_html=True)

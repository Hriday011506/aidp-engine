...
def monthly_weather(city):
    try:
        ...
        if not frame.empty:
            frame["month"]=frame.date.dt.month;grouped=frame.groupby("month").temp.mean();temps={calendar.month_name[m]:round(float(grouped.get(m,25)),1) for m in range(1,13)}
...
def live_market_price(product,city):
    """Use SerpApi shopping results directly; no retailer whitelist or product-title similarity filter."""
    key=str(st.secrets.get("SERPAPI_KEY","")).strip()
    if not key:return None,"SERPAPI_KEY is missing from Streamlit Secrets."
    params={"engine":"google_shopping_light","q":f"{product} price {city} India","location":f"{city}, India","hl":"en","gl":"in","device":"desktop","api_key":key}
    try:
        ...
        if data.get("error"):return None,f"SerpApi market lookup failed: {data['error']}"
        ...
        for item in results[:60]:
            ...
            priced.append({"price":price,"title":str(item.get("title") or "Shopping result"),"source":str(item.get("source") or item.get("merchant") or "SerpApi result")})
        if not priced:return None,f"SerpApi returned no priced results for {product} in {city}."
        selected=priced[:10];market=round(float(np.median([x["price"] for x in selected])),2);sources=[]
        for x in selected:
            if x["source"] not in sources:sources.append(x["source"])
        return market,f"SerpApi market results · {', '.join(sources[:6]) or 'shopping results'}"
    except requests.exceptions.Timeout:
        cached=st.session_state.get("price")
        if cached is not None:return float(cached),"Cached SerpApi market reference · live refresh timed out"
        return None,"SerpApi market lookup timed out. Please try again."
    except requests.exceptions.RequestException as exc:return None,f"SerpApi market lookup failed: {exc}"
    except (ValueError,TypeError) as exc:return None,f"SerpApi market data could not be read: {exc}"
...
def generate():
    ...
    table,city,weather_source=make_forecast(st.session_state.product,st.session_state.city);row=table.loc[table.Month==st.session_state.month].iloc[0];pred=float(row["Forecast demand"]);suggested=float(market*(1+np.clip((pred-300)/3000,-.08,.08)));p={"product":st.session_state.product,"city":city,"month":st.session_state.month,"pred":pred,"stock":float(np.ceil(pred*1.1)),"temp":float(row["Temperature (°C)"]),"holiday":int(row["Holiday days"]),"trend":int(row["Trend score"]),"market":float(market),"suggested":suggested,"source":weather_source,"confidence":int(np.clip(70+abs(int(row["Trend score"])-60)*.25,68,94))};st.session_state.forecast=table;st.session_state.prediction=p;st.session_state.saved.insert(0,p.copy());st.session_state.saved=st.session_state.saved[:20];return True

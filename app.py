...
def monthly_weather(city):
    try:
        geo = requests.get("https://geocoding-api.open-meteo.com/v1/search", params={"name": city, "count": 1, "language": "en", "format": "json"}, timeout=(3, 7))
        ...
        start = end - 4
        archive = requests.get("https://archive-api.open-meteo.com/v1/archive", params={"latitude": lat, "longitude": lon, "start_date": f"{start}-01-01", "end_date": f"{end}-12-31", "daily": "temperature_2m_mean", "timezone": "auto"}, timeout=(3, 12))
...
def _fallback_market_reference(product):
    p = str(product).lower()
    catalog = {
        "coke": (40.0, "Indicative reference · Coca-Cola family"),
        "coca cola": (40.0, "Indicative reference · Coca-Cola family"),
        "amul": (60.0, "Indicative reference · Amul family"),
...
def live_market_price(product, city):
    """Use SerpApi directly and accept any valid priced result; no retailer/product-title filtering."""
    key = str(st.secrets.get("SERPAPI_KEY", "")).strip()
    if not key:
        return None, "SERPAPI_KEY is missing from Streamlit Secrets."

    queries = [
        f"{product} price in {city} India",
    ...
    for query in queries:
        ...
        try:
            ...
            results += list(data.get("products_results") or [])
            priced = []
            for item in results[:80]:
                ...
                priced.append({
                    "price": price,
                    "title": str(item.get("title") or "Shopping result"),
                    "source": str(item.get("source") or item.get("merchant") or "SerpApi result"),
                })
            if priced:
                selected = priced[:10]
                market = round(float(np.median([x["price"] for x in selected])), 2)
                sources = []
    ...
    if cached is not None:
        return float(cached), "Cached market reference · SerpApi refresh failed"

    fallback, fallback_source = _fallback_market_reference(product)
    if fallback is not None:
        return fallback, fallback_source + " · live SerpApi unavailable"
...
def get_model():
    ...
    frame = pd.DataFrame({"holiday": rng.integers(0, 14, 800), "temp": rng.uniform(8, 42, 800), "trend": rng.integers(0, 100, 800)})
...
def make_forecast(product, city):
    ...
    for number, month in enumerate(MONTHS, 1):
        ...
        holiday = holiday_days(year, number)
...
def generate():
    ...
    if market is None:
        return False
    table, city, weather_source = make_forecast(st.session_state.product, st.session_state.city)
...
def normalize_prediction(p):
    ...
    if not required.issubset(p):
        return None
    try:
        for k in ["pred", "stock", "temp", "market", "suggested"]:
            p[k] = float(p[k])
        for k in ["holiday", "trend", "confidence"]:
            p[k] = int(p[k])
...
def action_list(p):
    ...
    if p["trend"] >= 78:
        a.append(("info", "Demand surge detected", "Trend score is strong; monitor replenishment more frequently."))
    if p["temp"] >= 34:
        a.append(("warning", "Watch heat sensitivity", "High temperature may increase volatility for sensitive products."))
    a.append(("warning", "Review pricing", "Demand-aware price is above the market reference.") if p["suggested"] > p["market"] * 1.02 else ("info", "Keep price near market", "Limited price change is indicated."))
    return a[:4]
    ...
    with x:
        if st.button("Refresh live market price", use_container_width=True, key="price_button"):
            ...
            if price is None:
                ...
                st.success(f"Market reference: ₹{price:,.2f} · {message}")
    with y:
        if st.button("Generate AI Decision →", use_container_width=True, key="generate_button"):
            if st.session_state.price is None:
                st.error("Refresh the market price first. A market reference is required for an AI decision.")
            elif generate():
                st.success("AI decision generated successfully.")
    if st.session_state.price is not None:
        st.markdown(f"<div style='background:#dcfce7;border-radius:12px;padding:14px 18px;color:#047857;font-weight:800'>Market reference: ₹{st.session_state.price:,.2f} · {html.escape(str(st.session_state.price_source or 'SerpApi market data'))}</div>", unsafe_allow_html=True)
...
def dashboard():
    p = normalize_prediction(st.session_state.prediction)
    cols = st.columns(4)
    values = [("FORECAST DEMAND", f"{int(p['pred']):,} units" if p else "—", "Current month"), ("RECOMMENDED STOCK", f"{int(p['stock']):,} units" if p else "—", "10% planning buffer"), ("MARKET REFERENCE", f"₹{p['market']:,.2f}" if p else "—", "SerpApi market data"), ("CONFIDENCE", f"{p['confidence']}%" if p else "—", "Model confidence")]
...
def pricing_page():
    header("DYNAMIC PRICING", "Price with <span>market awareness.</span>", "Compare the market reference with the demand-aware recommendation.")
    p = normalize_prediction(st.session_state.prediction)
    if not p:
        st.info("Generate a decision first.")
        return
    a, b, c = st.columns(3)
    a.metric("Market reference", f"₹{p['market']:,.2f}")
    b.metric("Suggested price", f"₹{p['suggested']:,.2f}")
    c.metric("Difference", f"{((p['suggested'] / p['market']) - 1) * 100:+.1f}%")
    st.markdown(f"<div class='decision'><h3>Pricing rationale</h3><p>Recommendation uses the market reference of ₹{p['market']:,.2f} and the current demand forecast of {int(p['pred']):,} units.</p></div>", unsafe_allow_html=True)
...
def settings_page():
    ...
    st.info("MongoDB and Brevo are disabled in this presentation build. Account data is kept in the Streamlit session.")

with st.sidebar:
    logo()
    if st.session_state.user:
        st.caption(st.session_state.user.get("email","Business User"))
        items=[("Dashboard","dashboard"),("Product Analysis","product"),("Demand Forecasting","forecast"),("Dynamic Pricing","pricing"),("Market Insights","market"),("Product Comparison","compare"),("AI Simulator","simulator"),("Alerts & Risk","alerts"),("AI Decision Copilot","copilot"),("Saved Analyses","saved"),("Settings","settings")]
...
if st.session_state.page=="welcome":
    header("OPTIRETAIL AI","Turn market data into <span>smarter decisions.</span>","Forecast demand, use SerpApi market intelligence, simulate scenarios, detect risks and explain AI recommendations.")
    ...
    with st.form("signup_form"):
        email=st.text_input("Email address");password=st.text_input("Password",type="password");gst=st.text_input("GST Number");turnover=st.selectbox("Annual Turnover",["1–5 Lakh","5–10 Lakh","10–15 Lakh","15–50 Lakh","50 Lakh+"]);submit=st.form_submit_button("Create account",use_container_width=True)
    if submit:
    ...
    if st.session_state.page=="dashboard":dashboard()
        ...
        header("MARKET INSIGHTS","SerpApi <span>market intelligence.</span>","SerpApi provides the current shopping-market reference used by the decision workflow.")

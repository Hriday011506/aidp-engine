...
def connect_mongo():
    """Fresh MongoDB connection. Never caches failed connections."""
    uri = MONGO_URI.strip()
    if not uri:
    ...
    try:
        ...
        if "?" in uri:
            ...
            uri = uri + "?" + params
        client = MongoClient(uri, serverSelectionTimeoutMS=7000, connectTimeoutMS=7000, socketTimeoutMS=7000, retryWrites=True, appname="OptiRetailAI")
        ...
        if "serverselectiontimeouterror" in name.lower():
            return None, "Atlas could not be reached. Check Atlas Network Access/IP allowlist."
        if "dns" in text or "querysrv" in name.lower():
            return None, "MongoDB DNS/SRV lookup failed. Check the Atlas cluster hostname."
        return None, f"MongoDB connection failed: {name}."

...
.hero{padding:36px 40px;border-radius:30px;background:linear-gradient(135deg,#ffffff 0%,#effbf6 100%);border:1px solid var(--line);box-shadow:0 22px 60px rgba(16,185,129,.08);margin-bottom:22px}
.hero h1{font-family:'Plus Jakarta Sans';font-size:2.75rem;line-height:1.07;letter-spacing:-2.1px;margin:0;color:#0f172a}
.hero h1 span{color:var(--green2)}.hero p{color:#64748b;font-size:1rem;max-width:950px;margin:.7rem 0 0}
.eyebrow{color:var(--green2);text-transform:uppercase;font-size:.65rem;font-weight:800;letter-spacing:2px;margin-bottom:9px}
...
.card h3{margin:.15rem 0 .45rem;font-family:'Plus Jakarta Sans';font-size:1.05rem}
...
.kpi small{color:#64748b;text-transform:uppercase;letter-spacing:1px;font-weight:800;font-size:.62rem}.kpi strong{display:block;font-family:'Plus Jakarta Sans';color:#0f172a;font-size:1.72rem;margin-top:7px}.kpi span{font-size:.75rem;color:#64748b}
...
.badge{display:inline-flex;align-items:center;gap:6px;padding:6px 10px;border-radius:999px;font-size:.65rem;font-weight:800}.badge.live{background:#ecfdf5;color:#047857;border:1px solid #bbf7d0}.badge.warn{background:#fff7ed;color:#c2410c;border:1px solid #fed7aa}
...
.auth-wrap{max-width:720px;margin:28px auto}.auth-card{background:rgba(255,255,255,.98);border:1px solid var(--line);border-radius:28px;padding:32px;box-shadow:0 20px 55px rgba(15,23,42,.06)}
.auth-title{font-family:'Plus Jakarta Sans';font-size:1.95rem;font-weight:800;letter-spacing:-1px;color:#0f172a;margin:0}.auth-sub{color:#64748b;margin:.4rem 0 1.4rem}
.stButton>button,.stFormSubmitButton>button{border:0!important;border-radius:12px!important;min-height:44px!important;font-weight:800!important;background:linear-gradient(135deg,#079568,#047a55)!important;color:#fff!important;box-shadow:0 8px 22px rgba(5,150,105,.16)}
.stButton>button:hover,.stFormSubmitButton>button:hover{transform:translateY(-1px)}
div[data-testid="stTextInput"] input,div[data-testid="stNumberInput"] input,div[data-testid="stSelectbox"] div[data-baseweb="select"]>div,div[data-testid="stTextArea"] textarea{background:#fff!important;color:#0f172a!important;border:1px solid #cfe0d9!important;border-radius:12px!important;caret-color:#047857!important;box-shadow:none!important}
...
label,div[data-testid="stWidgetLabel"] p{color:#334155!important;font-weight:600!important}
...
def mongo_ok():
    return db is not None and users_collection is not None

def generate_otp():
    return f"{secrets.randbelow(1_000_000):06d}"

def otp_is_valid(created_at, ttl_seconds=300):
    return bool(created_at) and (time.time() - created_at) <= ttl_seconds

def send_brevo_otp(email, otp):
    if not BREVO_API_KEY or not BREVO_SENDER_EMAIL:
        return False, "Email OTP is not configured. Add BREVO_API_KEY and BREVO_SENDER_EMAIL to Streamlit Secrets."
    try:
        response = requests.post(
            ...
            json={
                ...
                "textContent":f"Your OptiRetail AI verification code is {otp}. It expires in 5 minutes.",
                "htmlContent":f"<div style='font-family:Arial,sans-serif'><h2>OptiRetail AI</h2><p>Your verification code is:</p><div style='font-size:32px;font-weight:700;letter-spacing:6px'>{otp}</div><p>This code expires in 5 minutes.</p></div>"
            },
            timeout=10,
        )
        if 200 <= response.status_code < 300:
            return True, "OTP sent successfully."
        return False, "Brevo could not send the OTP. Verify your sender email and API key."
    except Exception:
...
def fetch_product_price(product_name):
    ...
    try:
        ...
        for item in results.get("shopping_results",[]):
            ...
            if "india" in title or "india" in source: score+=1
            candidates.append((score,value,item.get("title",""),item.get("source",""),item.get("link","")))
        if not candidates: return None
        candidates.sort(key=lambda x:x[0],reverse=True)
        top_score=candidates[0][0]; top=[c for c in candidates if c[0]==top_score][:5]
...
def send_otp_flow(email,password,gst,turnover):
    email=email.strip().lower()
    if not email or not password or not gst:
        st.error("Please fill in Email, Password and GST Number."); return
    if users_collection is None:
        st.error("MongoDB is unavailable. Check MONGO_URI in Streamlit Secrets."); return
    try:
        if users_collection.find_one({"email":email}):
            st.error("An account with this email already exists."); return
    except Exception:
        st.error("Unable to check the database right now."); return
    otp=generate_otp()
    ok,msg=send_brevo_otp(email,otp)
    if ok:
        st.session_state.otp_email=email
        st.session_state.otp_value=otp
        st.session_state.otp_created_at=time.time()
        st.session_state.pending_signup={"email":email,"password":password,"gst":gst,"turnover":turnover}
        st.session_state.otp_sent=True
        st.success(msg)
    else:
        st.error(msg)

def verify_otp_and_create(code):
    if not st.session_state.otp_sent or not otp_is_valid(st.session_state.otp_created_at):
        st.error("OTP expired. Please request a new one."); return
    if str(code).strip()!=st.session_state.otp_value:
        st.error("Incorrect OTP."); return
    data=st.session_state.pending_signup or {}
    ok,msg=signup(data.get("email",""),data.get("password",""),data.get("gst",""),data.get("turnover",""))
    if ok:
        st.success("Email verified and account created successfully.")
        st.session_state.page="login"; st.session_state.otp_sent=False; st.session_state.otp_value=""; st.session_state.pending_signup=None; st.rerun()
    else:
        st.error(msg)
...
with st.sidebar:
    ...
    if st.session_state.user:
        ...
        nav_items=[("Dashboard","dashboard"),("Product Analysis","product"),("Demand Forecasting","forecast"),("Dynamic Pricing","pricing"),("Market Insights","market"),("Saved Analyses","saved")]
...
if st.session_state.page=="welcome":
    ...
    with st.form("login_form"):
        ...
        if st.form_submit_button("Sign in",use_container_width=True):
            ...
            elif not mongo_ok(): st.error("MongoDB is unavailable. Check MONGO_URI in Streamlit Secrets.")
    ...
    st.markdown("<div class='auth-wrap'><div class='auth-card'><div class='eyebrow'>BUSINESS ONBOARDING</div><h2 class='auth-title'>Create your OptiRetail AI workspace</h2><p class='auth-sub'>Verify your email before the account is saved to MongoDB.</p>",unsafe_allow_html=True)
    with st.form("signup_form"):
        ...
        if st.form_submit_button("Send verification OTP",use_container_width=True):
            send_otp_flow(email,password,gst,turnover)
    if st.session_state.otp_sent:
        st.markdown("<div class='card'><div class='eyebrow'>EMAIL VERIFICATION</div><h3>Enter the 6-digit OTP</h3><p class='small-note'>We sent a verification code to your registered email. It expires in 5 minutes.</p></div>",unsafe_allow_html=True)
        with st.form("otp_form"):
            code=st.text_input("Verification code",max_chars=6,placeholder="123456")
            if st.form_submit_button("Verify email & create account",use_container_width=True): verify_otp_and_create(code)
        if st.button("Resend OTP",key="resend_otp"):
            data=st.session_state.pending_signup or {}; send_otp_flow(data.get("email",""),data.get("password",""),data.get("gst",""),data.get("turnover",""))
    ...
    mongo_badge="CONNECTED" if mongo_ok() else "UNAVAILABLE"; mongo_class="live" if mongo_ok() else "warn"
    serp_badge="CONFIGURED" if SERPAPI_KEY else "MISSING"; serp_class="live" if SERPAPI_KEY else "warn"
    brevo_badge="CONFIGURED" if BREVO_API_KEY and BREVO_SENDER_EMAIL else "MISSING"; brevo_class="live" if BREVO_API_KEY and BREVO_SENDER_EMAIL else "warn"
    st.markdown(f"<div class='card'><div class='eyebrow'>DATA SERVICES</div><h3>Connected services</h3><div class='signal'><b>MongoDB</b><span class='badge {mongo_class}'>{mongo_badge}</span></div><div class='signal'><b>SerpAPI</b><span class='badge {serp_class}'>{serp_badge}</span></div><div class='signal'><b>Brevo Email OTP</b><span class='badge {brevo_class}'>{brevo_badge}</span></div></div>",unsafe_allow_html=True)
    ...
    subtitles={"dashboard":"Enter a product and location, refresh the market reference, then generate a demand, inventory and pricing decision.","product":"Combine market price, weather, holidays and trend signals for a practical product decision.","forecast":"Forecast demand using the current weather, holiday and trend inputs.","pricing":"Use the market reference and predicted demand to calculate a bounded suggested selling price.","market":"Review live price reference together with environmental and demand signals.","saved":"Review your recent predictions stored in MongoDB."}
    ...
    if st.session_state.page=="saved":
        ...
        lp=st.session_state.get("last_prediction") or {}; current=st.session_state.get("current_price"); current_text=f"₹{current:,.2f}" if isinstance(current,(int,float)) else "—"; suggested_value=lp.get("suggested")
        ...
        if lp:
            ...
            with left:
                st.markdown("<div class='card'><div class='eyebrow'>DEMAND FORECAST</div><h3>12-month demand outlook</h3><div class='small-note'>Illustrative historical baseline and forecast trajectory</div>",unsafe_allow_html=True)
                months= list(calendar.month_name)[1:]; base=max(1,lp['pred']); x=np.arange(12); seasonal=1+0.10*np.sin((x+months.index(lp['month']))*2*np.pi/12); forecast=base*seasonal; hist=np.maximum(0,base*(0.82+0.08*np.sin((x+1)*2*np.pi/12))); chart=pd.DataFrame({"Month":months,"Historical demand":np.round(hist),"Forecast demand":np.round(forecast)}); st.line_chart(chart.set_index("Month"),height=320); st.markdown("</div>",unsafe_allow_html=True)
            with right:
                ...
                if lp['temp']>=32: action="Demand signal elevated by warm weather — keep additional safety stock for heat-sensitive categories."
                elif lp['temp']<=18: action="Cooler weather detected — demand may shift toward seasonal categories; keep inventory conservative."
                else: action="Weather is within a moderate range; holiday and trend signals drive the current decision."
                st.markdown(f"<div class='decision'><h3>Recommended action</h3><p>{action}</p></div>",unsafe_allow_html=True)
        else:
            st.markdown("<div class='card'><div class='eyebrow'>READY</div><h3>Generate your first AI decision</h3><p class='small-note'>Refresh the market reference, then click Generate AI Decision to populate demand, inventory, pricing and the forecast chart.</p></div>",unsafe_allow_html=True)

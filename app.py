...
MONGO_URI = read_secret("MONGO_URI") or read_secret("MONGODB_URI")
BREVO_API_KEY = read_secret("BREVO_API_KEY")
BREVO_SENDER_EMAIL = read_secret("BREVO_SENDER_EMAIL")
BREVO_SENDER_NAME = read_secret("BREVO_SENDER_NAME", "OptiRetail AI")
...
.auth{max-width:720px;margin:25px auto}.auth-card{background:#fff;border:1px solid var(--line);border-radius:28px;padding:34px;box-shadow:0 20px 55px rgba(15,23,42,.07)}.auth-title{font-size:2.1rem;font-weight:800;color:var(--green2);margin:0}.auth-sub{color:#64748b}
...
def get_mongo_client(uri, use_certifi):
    """Build (and cache) ONE MongoClient for this URI.

    Streamlit reruns the whole script on every click/keystroke. Without this
    cache, connect_mongo() used to call MongoClient(...) fresh every rerun,
    which opens a brand-new connection pool each time and never closes the
    old ones - on a shared/free Atlas cluster this quietly exhausts the
    connection limit and starts throwing errors after a bit of use.
    st.cache_resource keeps a single client alive across reruns, and (unlike
    st.cache_data) it does NOT cache a raised exception - so a failed
    connection is still retried on the next run, exactly like before.
    """
    kwargs = dict(
    ...
    if use_certifi:
        # Streamlit Cloud's system CA bundle is sometimes missing/outdated,
        # which makes the TLS handshake to Atlas fail with a certificate
        # verification error even though the URI/credentials are correct.
        kwargs["tlsCAFile"] = certifi.where()
    client = MongoClient(uri, **kwargs)
    client.admin.command("ping")  # forces the connection + auth to happen now
    return client


def connect_mongo():
    """Check/return the MongoDB connection. Never caches a failed attempt."""
    uri = MONGO_URI.strip()
    if not uri:
    ...
    try:
        ...
        if not parsed.hostname:
            return None, "MONGO_URI does not contain a valid MongoDB hostname."
        if "authSource=" not in uri:
            uri = uri + ("&" if "?" in uri else "?") + "authSource=admin"
        client = get_mongo_client(uri, certifi is not None)
        db = client["optiretail_ai"]
        return db, f"Connected to {parsed.hostname}"
    except Exception as exc:
        name = type(exc).__name__
        text = str(exc).lower()
        if "authentication failed" in text or "bad auth" in text or "authenticationfailure" in name.lower():
            return None, "MongoDB authentication failed. Check the Atlas database user, password and authSource=admin."
        if "serverselectiontimeouterror" in name.lower():
            return None, f"Atlas could not be reached. Check Atlas Network Access/IP allowlist (allow 0.0.0.0/0 for Streamlit Cloud). Details: {exc}"
        if "dns" in text or "querysrv" in name.lower():
            return None, f"MongoDB DNS/SRV lookup failed. Check the Atlas cluster hostname. Details: {exc}"
        if "ssl" in text or "tls" in text or "certificate" in text:
            return None, f"TLS/SSL handshake with Atlas failed. Details: {exc}"
        return None, f"MongoDB connection failed: {name}: {exc}"
...
def fetch_price(product):
    if not SERPAPI_KEY or GoogleSearch is None or not product.strip():
        return None
    try:
...
def get_weather(city):
    try:
        ...
        weather=requests.get("https://api.open-meteo.com/v1/forecast",params={"latitude":lat,"longitude":lon,"current_weather":"true"},timeout=5);weather.raise_for_status();return float(weather.json()["current_weather"]["temperature"])
...
def holiday_count(year, month):
    ...
    for day in range(1,calendar.monthrange(year,month)[1]+1):
        ...
        if d.weekday()>=5 or d in india: total+=1
    return total
...
def render_analysis():
    ...
    with a:
        if st.button("Refresh market price",use_container_width=True,key="refresh_price"):
            ...
            else: st.warning("No reliable shopping price found. Check SerpAPI or use a more specific product name.")
    with b:
        if st.button("Generate AI Decision →",use_container_width=True,key="generate_decision"):
            ...
            if predictions is not None and st.session_state.user:
                try:
                    ...
                    st.warning(f"Decision generated, but MongoDB could not save it: {type(exc).__name__}")
...
if st.session_state.page=="welcome":
    ...
    if submitted:
        ...
        elif users is None: st.error(f"MongoDB unavailable: {mongo_status}")
    ...
    st.markdown("<div class='auth'><div class='auth-card'><div class='eyebrow'>BUSINESS ONBOARDING</div><h1 class='auth-title'>Create your OptiRetail AI workspace</h1><p class='auth-sub'>Verify your email before the account is saved.</p>",unsafe_allow_html=True)
    if not st.session_state.otp_value:
        ...
        if send:
            ...
            if not email or "@" not in email or not password or not gst:
                ...
                try:
                    ...
                    exists=None;st.error(f"MongoDB error: {type(exc).__name__}")
                if exists:
        ...
        if verify:
            if time.time()-st.session_state.otp_created_at>300: st.error("OTP expired. Please resend a new code.")
        ...
        if st.button("Resend OTP",key="resend_otp"):
            ...
            new_code=generate_otp();ok,msg=send_otp(pending["email"],new_code)
            if ok:
    ...
    if predictions is None: st.warning(f"MongoDB unavailable: {mongo_status}")
    else:
        try:
            rows=list(predictions.find({"email":st.session_state.user.get("email")},{"_id":0}).sort("created_at",-1).limit(20))
            if rows: st.dataframe(pd.DataFrame(rows),use_container_width=True,hide_index=True)
            else: st.info("No saved analyses yet.")
        except Exception as exc: st.error(f"Could not load saved analyses: {type(exc).__name__}")

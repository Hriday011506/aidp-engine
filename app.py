...
def render_analysis():
    ...
    with a:
        if st.button("Refresh market price",use_container_width=True,key="refresh_price"):
            ...
            if data is None:
                st.session_state.current_price=None; st.session_state.market_data=None
                st.warning("No reliable shopping price found. Try a specific product name.")
            else:
                st.session_state.current_price=data["price"]; st.session_state.market_data=data
...
# ==============================
    ...
    if st.session_state.page=="saved":
        ...
        product,city,month_name=render_analysis()
        ...
        if lp:
            ...
            with c1:
                st.markdown("<div class='card'><div class='eyebrow'>DEMAND FORECAST</div><h3>Demand outlook</h3>",unsafe_allow_html=True)
                months= list(calendar.month_name)[1:]
                base=max(1,lp['pred'])
                # Deterministic 12-month outlook shaped around the selected forecast month.
                x=np.arange(12)
                seasonal=1+0.10*np.sin((x+months.index(lp['month']))*2*np.pi/12)

...
.stApp {
    background:
      ...
      radial-gradient(circle at 100% 10%, rgba(124,58,237,.16), transparent 26%),
...
[data-testid="stSidebar"] { background: linear-gradient(180deg,#030712,#0b1220); border-right:1px solid rgba(148,163,184,.12); }
...
.hero h1 { margin:0; font-size:3.15rem; line-height:1.05; letter-spacing:-1.8px; }
.hero p { color:#94a3b8; margin:10px 0 0; font-size:1rem; }
.eyebrow { color:#67e8f9; text-transform:uppercase; font-size:.72rem; font-weight:800; letter-spacing:2px; margin-bottom:10px; }
.card {
    background:rgba(15,23,42,.68); border:1px solid rgba(148,163,184,.12);
    border-radius:24px; padding:24px; box-shadow:0 18px 50px rgba(0,0,0,.22); margin-bottom:18px;
}
.kpi {
    background:linear-gradient(145deg,rgba(15,23,42,.96),rgba(30,41,59,.9));
    border:1px solid rgba(96,165,250,.13); border-radius:20px; padding:20px; min-height:125px;
    box-shadow:inset 0 1px rgba(255,255,255,.03);
}
.kpi small { color:#94a3b8; text-transform:uppercase; letter-spacing:1.2px; font-weight:700; }
.kpi strong { display:block; color:#f8fafc; font-size:1.85rem; margin-top:8px; }
.pill { display:inline-block; padding:7px 11px; border-radius:999px; background:rgba(34,197,94,.10); border:1px solid rgba(34,197,94,.2); color:#86efac; font-size:.72rem; font-weight:800; }
.metric-accent { color:#67e8f9; }
...
if st.session_state.page == "welcome":
    st.markdown("<div class='hero'><div class='eyebrow'>AI-POWERED RETAIL INTELLIGENCE</div><h1>Turn market signals into smarter decisions.</h1><p>AIDP combines demand intelligence, weather, holiday patterns and live market pricing in one business workspace.</p></div>", unsafe_allow_html=True)
    left, right = st.columns([1.35, .85], gap="large")
    with left:
        st.markdown("<div class='card'><span class='pill'>● SYSTEM READY</span><h2 style='margin-top:16px;'>One workspace. Four signals. Better decisions.</h2><p style='color:#94a3b8;line-height:1.8;'>Move from reactive inventory management to proactive demand planning with an executive-ready forecasting workflow.</p></div>", unsafe_allow_html=True)
        a,b,c = st.columns(3)
        a.metric("Demand", "AI Forecast")
        b.metric("Inventory", "+10% Buffer")
        c.metric("Market", "Live Signals")
    with right:
        st.markdown("<div class='card'><div class='eyebrow'>BUSINESS PORTAL</div><h2>Get started</h2><p style='color:#94a3b8;'>Create a business account or sign in to access your intelligence dashboard.</p></div>", unsafe_allow_html=True)
...
# ==============================
    ...
    st.markdown("<div class='hero'><div class='eyebrow'>SECURE BUSINESS ACCESS</div><h1>Welcome back.</h1><p>Sign in to your AIDP workspace.</p></div>", unsafe_allow_html=True)
...
# ==============================
    ...
    st.markdown("<div class='hero'><div class='eyebrow'>BUSINESS ONBOARDING</div><h1>Build your intelligence workspace.</h1><p>Set up your profile to start using AIDP.</p></div>", unsafe_allow_html=True)
...
# ==============================
    ...
    st.markdown("<div class='hero'><div class='eyebrow'>WORKSPACE</div><h1>Business settings</h1><p>Manage the profile used by your AIDP workspace.</p></div>", unsafe_allow_html=True)
    st.markdown("<div class='card'>", unsafe_allow_html=True)
    st.markdown("### Account profile")
    st.write(f"**Email:** {st.session_state.user.get('email','N/A')}")
    st.write(f"**GST:** {st.session_state.user.get('gst','N/A')}")
    st.write(f"**Turnover:** {st.session_state.user.get('turnover','N/A')}")
...
# ==============================
    ...
    st.markdown("<div class='hero'><div class='eyebrow'>LIVE INTELLIGENCE CONSOLE</div><h1>Good business decisions start with good signals.</h1><p>Monitor your market context, generate a demand forecast and turn it into an inventory action.</p></div>", unsafe_allow_html=True)
    ...
    k3.markdown(f"<div class='kpi'><small>Trend Score</small><strong>{viral}</strong></div>",unsafe_allow_html=True)
    ...
    if st.button("Generate demand forecast", use_container_width=True):

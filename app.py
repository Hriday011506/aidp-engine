...
def _tokens(text): return [x for x in re.findall(r"[a-z0-9]+",str(text).lower()) if len(x)>1]
def _canonical_tokens(text): return [{"coke":"coca","cocacola":"coca","cola":"coca"}.get(x,x) for x in _tokens(text)]
def _product_score(query,title):
    q=_canonical_tokens(query);t=_canonical_tokens(title)
    if not q or not t:return 0.0
    exact=sum(x in t for x in q)/len(q);fuzzy=max((SequenceMatcher(None,x,y).ratio() for x in q for y in t),default=0);return min(1.0,.65*exact+.35*fuzzy)

def _fallback_market_reference(product):
    p=" ".join(_canonical_tokens(product));catalog=[(("coca",),40.0,"Indicative reference · Coca-Cola family"),(("amul",),56.0,"Indicative reference · Amul family"),(("pepsi",),45.0,"Indicative reference · Pepsi family"),(("lays",),20.0,"Indicative reference · Lay's family"),(("rice",),320.0,"Indicative reference · Rice 5kg"),(("cooking","oil"),145.0,"Indicative reference · Cooking Oil 1L"),(("wheat","flour"),260.0,"Indicative reference · Wheat Flour 5kg")]
    for keys,price,label in catalog:
        if all(k in p for k in keys):return price,label
    return None,None
...
def live_market_price(product,city):
    ...
    queries=[f"{product} price in {city} India",f"{product} MRP price India",f"{product} current price India"];last_error=None
    for query in queries:
        ...
        try:
            ...
            for item in list(data.get("shopping_results") or [])[:30]+list(data.get("organic_results") or [])[:30]:
                ...
                if price is None:
                    text=f"{item.get('title','')} {item.get('snippet','')} {item.get('rich_snippet','')}";m=re.findall(r"(?:₹|Rs\.?|INR)\s*([0-9]+(?:\.[0-9]+)?)",text,flags=re.I)
                    if m:price=_price_number(m[0])
                if price is None or not 0<price<100000:continue
                title=str(item.get("title") or "");relevance=_product_score(product,title);source=str(item.get("source") or item.get("merchant") or item.get("displayed_link") or "Web result")
                candidates.append((relevance,price,source))
            if candidates:
...
if st.session_state.page=="welcome":
    ...
    if st.session_state.page=="dashboard":header("DASHBOARD","Turn market data into <span>smarter decisions.</span>","AI Action Center, health score, explainable forecast and simulator.");dashboard()
        ...
        header("MARKET INSIGHTS","Live <span>web market intelligence.</span>","The current market reference is required for the decision workflow.");price,message=live_market_price(st.session_state.product,st.session_state.city)

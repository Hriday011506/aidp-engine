...
DEFAULTS = {"user": None, "users": {}, "page": "welcome", "product": "Wheat Flour", "city": "Jaipur", "month": dt.datetime.now().strftime("%B"), "prediction": None, "forecast": None, "price": None, "price_source": None, "social_score": None, "social_source": None, "saved": []}
...
def live_market_price(product,city):
    ...
    for query in queries:
        ...
        try:
            ...
            for item in pools[:100]:
                price=_price_number(item.get("extracted_price",item.get("price")))
                if price is None:
                    text=f"{item.get('title','')} {item.get('snippet','')} {item.get('rich_snippet','')}"
                    found=re.findall(r"(?:₹|Rs\.?|INR)\s*([0-9]+(?:\.[0-9]+)?)",text,flags=re.I)
                    if found: price=_price_number(found[0])
                if price is None or price <= 0: continue
                source=str(item.get("source") or item.get("merchant") or item.get("displayed_link") or "Google result")
                candidates.append((float(price),source))
            if candidates:
                # Use the most central of the prices returned by Google to reduce outlier influence
                ...
                return market,f"Live Google market reference · {', '.join(sources[:8]) or 'Google web results'}"
            last_error="Google returned no usable priced results."
        except requests.exceptions.Timeout:last_error="Google web lookup timed out."
...
def _social_fallback(product):
    """Deterministic presentation-safe social demand signal when no social API is connected."""
    seed=sum((i+1)*ord(ch) for i,ch in enumerate(str(product).lower()))
    return int(52+(seed%44))

def social_viral_score(product):
    """Return a 0-100 Social Viral Score. Uses live Google trend/news signals when available, otherwise clearly labeled deterministic demo signal."""
    key=str(st.secrets.get("SERPAPI_KEY","")).strip()
    if key:
        query=f"{product} trending viral social media India"
        params={"engine":"google","q":query,"hl":"en","gl":"in","device":"desktop","api_key":key}
        try:
            response=requests.get("https://serpapi.com/search.json",params=params,timeout=(3,7));response.raise_for_status();data=response.json()
            text=" ".join([str(x.get("title",""))+" "+str(x.get("snippet","")) for x in (data.get("organic_results") or [])[:20]]).lower()
            keywords=["viral","trending","trend","instagram","reels","tiktok","youtube","twitter","x.com","social","shares","likes","meme","buzz"]
            hits=sum(text.count(k) for k in keywords)
            score=int(np.clip(45+hits*4,45,95))
            return score,"Google trend/social signal"
        except Exception: pass
    return _social_fallback(product),"Demo social-trend signal · no social API connected"
...
def get_model():
    rng=np.random.default_rng(42);frame=pd.DataFrame({"holiday":rng.integers(0,14,800),"temp":rng.uniform(8,42,800),"trend":rng.integers(0,100,800),"social":rng.integers(40,96,800)});season=np.maximum(0,24-np.abs(frame.temp-28));frame["sales"]=170+frame.holiday*38+frame.trend*4.7+season*12+(frame.social-60)*4+rng.normal(0,24,800);model=RandomForestRegressor(n_estimators=220,max_depth=12,random_state=42,n_jobs=-1);model.fit(frame[["holiday","temp","trend","social"]],frame.sales);return model
MODEL=get_model()
def make_forecast(product,city):
    ...
    st.session_state.social_score=social_score;st.session_state.social_source=social_source
...
def generate():
    ...
    pred=float(row["Forecast demand"]);temp=float(row["Temperature (°C)"]);holiday=int(row["Holiday days"]);trend_score=int(row["Trend score"]);social_score=int(row["Social Viral Score"])
    demand_factor=float(np.clip((pred-700)/5000,-0.045,0.045));weather_factor=float(np.clip((temp-27)/250,-0.025,0.025));holiday_factor=float(np.clip(holiday/600,-0.012,0.012));trend_factor=float(np.clip((trend_score-60)/3000,-0.015,0.015));social_factor=float(np.clip((social_score-60)/1000,-0.025,0.025))
    adjustment=float(np.clip(demand_factor+weather_factor+holiday_factor+trend_factor+social_factor,-0.075,0.075));base=market*(1+adjustment);mrp=_mrp_for_product(st.session_state.product,market);suggested=round(min(base,mrp),2) if mrp is not None else round(base,2)
    p={"product":st.session_state.product,"city":city,"month":st.session_state.month,"pred":pred,"stock":float(np.ceil(pred*1.1)),"temp":temp,"holiday":holiday,"trend":trend_score,"social":social_score,"market":float(market),"suggested":suggested,"source":weather_source,"social_source":st.session_state.social_source or "Social demand signal","confidence":int(np.clip(70+abs(trend_score-60)*.20+abs(social_score-60)*.10,68,96))}
...
def normalize_prediction(p):
    ...
    try:
        ...
        p["social"]=int(p.get("social",60));p["social_source"]=str(p.get("social_source","Social demand signal"));p["source"]=str(p.get("source","Historical weather"));return p
...
def health(p):
    if not p:return 0
    score=(np.clip(65+(p["trend"]-50)*.35,50,94)+np.clip(92-max(0,(p["pred"]-1200)/40),60,94)+80+np.clip(92-abs(p["temp"]-27)*1.4,60,94)+np.clip(70+(p.get("social",60)-50)*.35,55,94))/5;return int(np.clip(score,55,95))
def action_list(p):
    ...
    if p.get("social",60)>=80:a.append(("info","Social trend detected","Elevated social interest may increase near-term demand."))
...
def inputs():
    ...
    if changed:st.session_state.prediction=None;st.session_state.forecast=None;st.session_state.price=None;st.session_state.price_source=None;st.session_state.social_score=None;st.session_state.social_source=None
    x,y=st.columns(2)
    with x:
    ...
    if st.session_state.social_score is not None:st.markdown(f"<div style='background:#f0fdf4;border-radius:12px;padding:12px 18px;color:#047857;font-weight:800;margin-top:10px'>Social Viral Score: {st.session_state.social_score}/100 · {html.escape(str(st.session_state.social_source or 'Social demand signal'))}</div>",unsafe_allow_html=True)
...
def dashboard():
    p=normalize_prediction(st.session_state.prediction);cols=st.columns(5);values=[("FORECAST DEMAND",f"{int(p['pred']):,} units" if p else "—","Current month"),("RECOMMENDED STOCK",f"{int(p['stock']):,} units" if p else "—","10% planning buffer"),("MARKET REFERENCE",f"₹{p['market']:,.2f}" if p else "—","Live web market data"),("SOCIAL VIRAL",f"{p.get('social',0)}/100" if p else "—","Demand signal"),("CONFIDENCE",f"{p['confidence']}%" if p else "—","Model confidence")]
    ...
    with b:
        ...
        for name,value in [("Product trend",f"{p['trend']}/100"),("Holiday days",p["holiday"]),("Temperature",f"{p['temp']:.1f} °C"),("Social Viral Score",f"{p.get('social',60)}/100"),("Confidence",f"{p['confidence']}%")]:st.markdown(f"<p class='small'><b>{name}</b> <span style='float:right'>{value}</span></p>",unsafe_allow_html=True)
        st.markdown("</div>",unsafe_allow_html=True)
    simulator()
def product_page():
...
def pricing_page():
    ...
    a,b,c=st.columns(3);a.metric("Market reference",f"₹{p['market']:,.2f}");b.metric("Suggested price",f"₹{p['suggested']:,.2f}");c.metric("Difference",f"{((p['suggested']/p['market'])-1)*100:+.1f}%");st.markdown(f"<div class='decision'><h3>Pricing rationale</h3><p>Recommendation uses market price plus demand, temperature, holidays, product trend and social trend signals. Social Viral Score: <b>{p.get('social',60)}/100</b>.</p></div>",unsafe_allow_html=True)
def compare_page():
    ...
    if st.button("Compare products →",use_container_width=True,key="compare_button"):
        ...
        for product in [p1,p2,p3]:
            if product.strip():table,resolved_city,_=make_forecast(product,st.session_state.city);peak=table.loc[table["Forecast demand"].idxmax()];rows.append({"Product":product,"City":resolved_city,"Social Viral Score":int(table["Social Viral Score"].iloc[0]),"12-month demand":int(table["Forecast demand"].sum()),"Peak month":peak.Month,"Peak demand":int(peak["Forecast demand"]),"Recommended stock":int(np.ceil(table["Forecast demand"].sum()*1.1))})
...
def copilot_page():
    ...
    if st.button("Analyze question →",use_container_width=True,key="ask"):
        ...
        if not p:st.info("Generate a decision first.")
            ...
            elif "social" in query or "viral" in query or "trend" in query:answer=f"The Social Viral Score is {p.get('social',60)}/100. This signal is included in the demand forecast and pricing recommendation."
            elif "price" in query:answer=f"The market reference is ₹{p['market']:,.2f}; the demand-aware recommendation is ₹{p['suggested']:,.2f}."
            else:answer=f"The model forecasts {int(p['pred']):,} units with {p['confidence']}% confidence. Trend is {p['trend']}/100, social signal is {p.get('social',60)}/100 and temperature is {p['temp']:.1f} °C."
...
if st.session_state.page=="welcome":
    ...
    if st.session_state.page=="dashboard":header("DASHBOARD","Turn market data into <span>smarter decisions.</span>","AI Action Center, health score, explainable forecast and simulator.");dashboard()
        ...
        for saved in st.session_state.saved[:10]:
            ...
            if p:st.markdown(f"<div class='card'><h3>{html.escape(str(p['product']))} · {html.escape(str(p['city']))} · {p['month']}</h3><p class='small'>Demand {int(p['pred']):,} · Stock {int(p['stock']):,} · Market ₹{p['market']:,.2f} · Social {p.get('social',60)}/100 · Confidence {p['confidence']}%</p></div>",unsafe_allow_html=True)

...
def get_database():
    """Create a fresh client from current Streamlit Secrets on every app run."""
    if not MONGO_URI:
    ...
    try:
        ...
        text = str(exc)
        low = text.lower()
        if "authentication failed" in low or "bad auth" in low or "authenticationfailed" in low:
            return None, "MongoDB authentication failed. Verify the Atlas database user, password and authSource."
        ...
        if "dns" in low or "querysrv" in name.lower():
            return None, "MongoDB DNS/SRV lookup failed. Check the Atlas cluster hostname in MONGO_URI."
        return None, f"MongoDB connection failed: {name}. Check the URI and Atlas configuration."
    ...
    try:
        response = requests.post(
            ...
            json={
                ...
                "htmlContent":f"<div style='font-family:Arial;padding:24px'><h2>Verify your OptiRetail AI account</h2><p>Your verification code is:</p><div style='font-size:32px;font-weight:700;letter-spacing:8px;color:#047857'>{otp}</div><p>This code expires in 5 minutes.</p></div>",
            },
            timeout=10,
        )
        ...
        if send:
            ...
            if not email or "@" not in email or not password or not gst:
                ...
                try:
                    ...
                    exists=None;st.error(f"MongoDB error: {type(exc).__name__}: {exc}")
                if not exists:
    ...
    if st.session_state.page=="forecast":
        ...
        if predictions is None:
            ...
            try:
                ...
                st.error(f"Could not load saved analyses: {type(exc).__name__}: {exc}")

...
:root{--ink:#10231f;--muted:#6b7b76;--green:#07865f;--green2:#056b4c;--line:#dcebe5;--soft:#f5fbf8}
...
.auth-wrap{max-width:720px;margin:28px auto}.auth-card{background:rgba(255,255,255,.99);border:1px solid var(--line);border-radius:28px;padding:36px;box-shadow:0 20px 55px rgba(15,23,42,.06)}
.auth-title{font-family:'Plus Jakarta Sans';font-size:2rem;font-weight:800;letter-spacing:-1px;color:#0f172a;margin:0}.auth-sub{color:#64748b;margin:.4rem 0 1.4rem}
...
div[data-testid="stTextInput"] input,div[data-testid="stNumberInput"] input,div[data-testid="stSelectbox"] div[data-baseweb="select"]>div,div[data-testid="stTextArea"] textarea{background:#ffffff!important;color:#0f172a!important;border:1px solid #cfe0d9!important;border-radius:12px!important;caret-color:#047857!important;box-shadow:none!important}
...
def get_database():
    if not MONGO_URI or MongoClient is None:
        return None, "MONGO_URI missing or PyMongo unavailable"
    try:
...
# EMAIL OTP
# ==============================
from src.email_otp import generate_otp, otp_is_valid

def send_otp_email(to_email, otp):
    if not BREVO_API_KEY or not BREVO_SENDER_EMAIL:
        return False, "Brevo email settings are missing in Streamlit Secrets."
    try:
        response = requests.post(
            ...
            json={
                ...
                "htmlContent":f"<div style='font-family:Arial,sans-serif;padding:24px'><h2>Verify your OptiRetail AI account</h2><p>Your verification code is:</p><div style='font-size:32px;font-weight:700;letter-spacing:8px;color:#047857'>{otp}</div><p>This code expires in 5 minutes.</p></div>",
            },
            timeout=10,
        )
        if 200 <= response.status_code < 300:
            return True, "OTP sent to your email."
        return False, f"Email service error ({response.status_code}). Verify your Brevo sender settings."
    except Exception as exc:
...
def save_user(email, password, gst, turnover):
    if users_collection is None:
        return False, "MongoDB is unavailable. Check MONGO_URI and Atlas Network Access."
    try:
        if users_collection.find_one({"email": email.strip().lower()}):
...
def login(email,password):
    ...
    try:
        return users_collection.find_one({"email":email.strip().lower(),"password":password})
...
def fetch_product_price(product_name):
    ...
    try:
        result=GoogleSearch({"engine":"google_shopping_light","q":f"{product_name} price","gl":"in","hl":"en","num":8,"api_key":SERPAPI_KEY}).get_dict()
        ...
        for item in result.get("shopping_results",[]):
            ...
            try: value=float(str(raw).replace(",","").replace("₹","").strip())
            except (TypeError,ValueError): continue
            if value<=0: continue
            title=str(item.get("title","" )).lower(); source=str(item.get("source","")).lower()
            score=sum(tok in title for tok in tokens)*3+(10 if product_name.lower() in title else 0)+(1 if "india" in title or "india" in source else 0)
            candidates.append((score,value,item.get("title",""),item.get("source",""),item.get("link","")))
        if not candidates:return None
        candidates.sort(reverse=True); best=candidates[0][0]; top=[x for x in candidates if x[0]==best][:5]
        return {"price":float(np.median([x[1] for x in top])),"results":top}
    except Exception:return None
...
def load_data():
    ...
    df["sales"]=180+df["holiday_count"]*42+df["viral_score"]*4.8+seasonal*11+rng.normal(0,28,240)
    return df

@st.cache_resource
def train_model(df):
    m=RandomForestRegressor(n_estimators=250,max_depth=12,min_samples_leaf=2,random_state=42,n_jobs=-1)
...
with st.sidebar:
    ...
    if st.session_state.user:
        ...
        nav=[("Dashboard","dashboard"),("Product Analysis","product"),("Demand Forecasting","forecast"),("Dynamic Pricing","pricing"),("Market Insights","market"),("Saved Analyses","saved")]
...
# ==============================
    ...
    if not st.session_state.otp_value:
            ...
            if st.form_submit_button("Send verification OTP",use_container_width=True):
                ...
                if not email or not password or not gst: st.error("Please fill in Email, Password and GST Number.")
                    ...
                    if existing: st.error("An account with this email already exists.")
                        ...
                        if ok:
                            st.session_state.otp_email=email; st.session_state.otp_value=otp; st.session_state.otp_created_at=datetime.datetime.now().timestamp(); st.session_state.pending_signup={"email":email,"password":password,"gst":gst,"turnover":turnover}; st.rerun()
                        else: st.error(msg)
    else:
        st.success(f"Verification code sent to {st.session_state.otp_email}")
        otp_input=st.text_input("6-digit OTP",max_chars=6,placeholder="000000")
        if st.button("Verify email & create account",use_container_width=True,key="verify_otp"):
            ...
            if not created or not otp_is_valid(created): st.error("OTP expired. Please request a new one.")
                ...
                if ok:
                    st.session_state.otp_value=None; st.session_state.otp_email=None; st.session_state.otp_created_at=None; st.session_state.pending_signup=None; st.success(msg); st.session_state.page="login"; st.rerun()
                else: st.error(msg)
        if st.button("Resend OTP",key="resend_otp"):
            ...
            if ok: st.session_state.otp_value=new_otp; st.session_state.otp_created_at=datetime.datetime.now().timestamp(); st.success("New OTP sent."); st.rerun()
            else: st.error(msg)
    ...
    if st.button("← Back to home",key="signup_back"):
        st.session_state.otp_value=None; st.session_state.pending_signup=None; st.session_state.page="welcome"; st.rerun()
    st.markdown("</div></div>",unsafe_allow_html=True)
...
# ==============================
    ...
    st.markdown("<div class='hero'><div class='eyebrow'>WORKSPACE</div><h1>Business settings</h1><p>Manage your OptiRetail AI workspace and connected services.</p></div>",unsafe_allow_html=True)
    ...
    mongo_class="live" if db is not None else "warn"; mongo_badge="CONNECTED" if db is not None else "UNAVAILABLE"
    st.markdown(f"<div class='card'><div class='eyebrow'>DATA SERVICES</div><h3>Connected services</h3><div class='signal'><b>MongoDB</b><span class='badge {mongo_class}'>{mongo_badge}</span></div><div class='signal'><b>SerpAPI</b><span class='badge {'live' if SERPAPI_KEY else 'warn'}'>{'CONFIGURED' if SERPAPI_KEY else 'MISSING'}</span></div><div class='signal'><b>Brevo Email OTP</b><span class='badge {'live' if BREVO_API_KEY and BREVO_SENDER_EMAIL else 'warn'}'>{'CONFIGURED' if BREVO_API_KEY and BREVO_SENDER_EMAIL else 'MISSING'}</span></div><div class='small-note' style='margin-top:10px'>{mongo_status}</div></div>",unsafe_allow_html=True)
...
def render_analysis():
    ...
    with b:
        if st.button("Generate AI Decision →",use_container_width=True,key="generate_decision"):
            month_num=months.index(month_name)+1; year=datetime.datetime.now().year; holiday_count=get_holidays(year,month_num); temperature=get_weather(city); trend=trend_score(product)
            input_df=pd.DataFrame({"holiday_count":[holiday_count],"avg_temp":[temperature],"viral_score":[trend]})
            predicted=float(max(0,model.predict(input_df)[0])); stock=float(np.ceil(predicted*1.10)); market=st.session_state.get("current_price")
...
# ==============================
    ...
    titles={"dashboard":"Good morning. Make the next decision","product":"Analyze a product","forecast":"Demand forecasting","pricing":"Dynamic pricing","market":"Market insights","saved":"Saved analyses"}
    ...
    if st.session_state.page=="saved":
        ...
        product,city,month_name=render_analysis(); lp=st.session_state.get("last_prediction") or {}; current=st.session_state.get("current_price"); current_text=f"₹{current:,.2f}" if isinstance(current,(int,float)) else "—"; suggested_value=lp.get("suggested")

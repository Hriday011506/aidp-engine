...
def _tokens(text):
    return [x for x in re.findall(r"[a-z0-9]+", str(text).lower()) if len(x)>1]


def _canonical_tokens(text):
    aliases={"coke":"coca","cocacola":"coca","cola":"coca","pepsi":"pepsi","lays":"lays","lay's":"lays"}
    return [aliases.get(x,x) for x in _tokens(text)]


def _product_score(query,title):
    q=_canonical_tokens(query); t=_canonical_tokens(title)
    if not q or not t:return 0.0
    exact=sum(x in t for x in q)/len(q)
    fuzzy=max((SequenceMatcher(None,x,y).ratio() for x in q for y in t),default=0)
    brands={"coca","amul","pepsi","britannia","parle","nestle","haldiram","surf","tata","fortune","aashirvaad"}
    qb=brands.intersection(q); tb=brands.intersection(t)
    bonus=.25 if qb and tb and qb.intersection(tb) else 0.0
...
def get_model():
    rng=np.random.default_rng(42); frame=pd.DataFrame({"holiday":rng.integers(0,14,800),"temp":rng.uniform(8,42,800),"trend":rng.integers(0,100,800)})
    season=np.maximum(0,24-np.abs(frame.temp-28)); frame["sales"]=170+frame.holiday*38+frame.trend*4.7+season*12+rng.normal(0,24,800)
    model=RandomForestRegressor(n_estimators=220,max_depth=12,random_state=42,n_jobs=-1); model.fit(frame[["holiday","temp","trend"]],frame.sales); return model
...
def make_forecast(product,city):
    ...
    for number,month in enumerate(MONTHS,1):
        temp=float(temps.get(month,25)); holiday=holiday_days(year,number); features=pd.DataFrame({"holiday":[holiday],"temp":[temp],"trend":[product_trend]}); demand=float(max(0,MODEL.predict(features)[0]))
...
def generate():
    ...
    table,city,weather_source=make_forecast(st.session_state.product,st.session_state.city); row=table.loc[table.Month==st.session_state.month].iloc[0]; pred=float(row["Forecast demand"]); suggested=float(market*(1+np.clip((pred-300)/3000,-.08,.08)))
...
def action_list(p):
    ...
    a.append(("warning","Review pricing","Demand-aware price is above the live market reference.") if p["suggested"]>p["market"]*1.02 else ("info","Keep price near market","Limited price change is indicated.")); return a[:4]
...
if st.session_state.page=="welcome":
    header("OPTIRETAIL AI","Turn market data into <span>smarter decisions.</span>","Forecast demand, use live retailer intelligence, simulate scenarios, detect risks and explain AI recommendations.");a,b,c,d=st.columns(4)
    ...
    if st.session_state.page=="dashboard":header("DASHBOARD","Turn market data into <span>smarter decisions.</span>","AI Action Center, health score, explainable forecast and simulator.");dashboard()
        ...
        header("MARKET INSIGHTS","Live retailer <span>intelligence.</span>","The current market reference is required for the decision workflow.");price,message=live_market_price(st.session_state.product,st.session_state.city)

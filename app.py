...
[data-testid="stSidebar"]{background:linear-gradient(180deg,#062b22,#07372c);border-right:1px solid rgba(255,255,255,.08)}
[data-testid="stSidebar"] *{color:#ecfdf5}
.sidebar-logo{font-family:'Plus Jakarta Sans';font-size:1.65rem;font-weight:800;letter-spacing:-.8px}
...
.hero h1{font-family:'Plus Jakarta Sans';font-size:2.9rem;line-height:1.07;letter-spacing:-2.1px;margin:0;color:#0f172a}
.hero h1 span{color:#047857}.hero p{color:#64748b;font-size:1rem;max-width:920px;margin:.72rem 0 0}
.eyebrow{color:#047857;text-transform:uppercase;font-size:.66rem;font-weight:800;letter-spacing:2px;margin-bottom:9px}
.section-title{font-family:'Plus Jakarta Sans';font-size:1.18rem;font-weight:800;color:#0f172a;margin:.4rem 0 .95rem}
...
.kpi small{color:#64748b;text-transform:uppercase;letter-spacing:1px;font-weight:800;font-size:.63rem}.kpi strong{display:block;font-family:'Plus Jakarta Sans';color:#0f172a;font-size:1.75rem;margin-top:7px}.kpi span{font-size:.75rem;color:#64748b}
.badge{display:inline-flex;align-items:center;gap:6px;padding:6px 10px;border-radius:999px;font-size:.66rem;font-weight:800}.badge.live{background:#ecfdf5;color:#047857;border:1px solid #bbf7d0}.badge.info{background:#eff6ff;color:#1d4ed8;border:1px solid #bfdbfe}
...
# Deterministic demo trend signal; keep clear in UI that this is a model signal, not a live social crawl.
@st.cache_data
...
def load_data():
    ...
    # Weather is deliberately influential in the prototype training relation.
...
# ==============================
    ...
    st.markdown("<div class='card'><div class='eyebrow'>DATA SERVICES</div><h3>Connected services</h3><div class='signal'><b>MongoDB</b><span class='badge live'>CONNECTED</span></div><div class='signal'><b>SerpAPI</b><span class='badge live'>CONFIGURED</span></div></div>", unsafe_allow_html=True)

# ==============================
# DASHBOARD
# ==============================
elif st.session_state.page == "dashboard" and st.session_state.user:
    st.markdown("<div class='hero'><div class='eyebrow'>RETAIL INTELLIGENCE CONSOLE</div><h1>Make the next decision <span>with confidence.</span></h1><p>Enter a product and location, refresh the market reference, then generate a demand, inventory and pricing decision.</p></div>", unsafe_allow_html=True)
    ...
    with c2:
        city = st.text_input("City", value=st.session_state.city, label_visibility="collapsed")
    with c3:
    ...
    k1.markdown(f"<div class='kpi'><small>Forecasted demand</small><strong>{int(lp['pred']):,} units</strong><span>AI demand estimate</span></div>" if lp else "<div class='kpi'><small>Forecasted demand</small><strong>—</strong><span>Generate a decision</span></div>", unsafe_allow_html=True)
    k2.markdown(f"<div class='kpi'><small>Recommended stock</small><strong>{int(lp.get('stock', lp.get('inventory', 0))):,} units</strong><span>Includes 10% planning buffer</span></div>" if lp else "<div class='kpi'><small>Recommended stock</small><strong>—</strong><span>Inventory action</span></div>", unsafe_allow_html=True)
    ...
    k4.markdown(f"<div class='kpi'><small>Suggested price</small><strong>₹{suggested_value:,.2f}</strong><span>Demand-response guidance</span></div>" if isinstance(suggested_value, (int, float)) else "<div class='kpi'><small>Suggested price</small><strong>—</strong><span>Refresh market price</span></div>", unsafe_allow_html=True)

    if lp:
        left, right = st.columns([1.55, 1])
        with left:
            st.markdown("<div class='card'><div class='eyebrow'>DEMAND FORECAST</div><h2 style='margin:0 0 4px'>12-month demand outlook</h2><div class='small-note'>Historical baseline and forecast trajectory</div>", unsafe_allow_html=True)
            month_labels = ["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"]
            base = max(lp["pred"] * 0.82, 1)
            seasonal = np.array([0.88,0.91,0.96,1.00,0.93,0.98,1.03,1.08,1.14,1.10,1.04,1.07])
            historical = base * np.array([0.90,0.93,0.95,0.98,1.0])
            future = lp["pred"] * (seasonal / seasonal.mean())
            chart = pd.DataFrame(index=month_labels)
            chart["Historical Demand"] = np.nan
            chart["Forecasted Demand"] = np.nan
            chart.loc[month_labels[:5], "Historical Demand"] = historical
            chart.loc[month_labels[4:], "Forecasted Demand"] = future[4:]
            st.line_chart(chart, use_container_width=True, height=330)
            st.markdown("</div>", unsafe_allow_html=True)
        with right:
            st.markdown("<div class='card'><div class='eyebrow'>PRICE OPTIMIZATION</div><h2 style='margin:0 0 4px'>Pricing guidance</h2><div class='small-note'>Market reference adjusted by demand signal</div>", unsafe_allow_html=True)
            if isinstance(lp.get("suggested"), (int, float)) and isinstance(current, (int, float)):
                low, high = current * 0.92, current * 1.08
                st.metric("Suggested price", f"₹{lp['suggested']:,.2f}")
                st.caption(f"Reference band: ₹{low:,.0f} – ₹{high:,.0f}")
                st.progress(float(np.clip((lp['suggested'] - low) / (high - low), 0, 1)))
            else:
                st.info("Refresh the market price to unlock pricing guidance.")
            st.markdown("</div>", unsafe_allow_html=True)

        st.markdown("<div class='card'><div class='eyebrow'>MARKET & DEMAND SIGNALS</div><h2 style='margin:0 0 8px'>What is driving the recommendation?</h2>", unsafe_allow_html=True)
        s1, s2, s3 = st.columns(3)
        s1.markdown(f"<div class='signal'><b>Temperature</b><br><small>{lp['temp']:.1f}°C in {city}</small> <span class='badge live'>WEATHER</span></div>", unsafe_allow_html=True)
        s2.markdown(f"<div class='signal'><b>Holiday activity</b><br><small>{lp['holiday']} days in {month_name}</small> <span class='badge info'>CALENDAR</span></div>", unsafe_allow_html=True)
        s3.markdown(f"<div class='signal'><b>Demand trend</b><br><small>{lp['trend']}/100 model signal</small> <span class='badge info'>MODEL</span></div>", unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)

        if lp["trend"] >= 70:
            recommendation = "High demand momentum detected. Increase replenishment readiness and review the suggested selling price."
        elif lp["holiday"] >= 8:
            recommendation = "Elevated holiday activity detected. Hold additional stock ahead of the forecast month."
        else:
            recommendation = "Demand conditions appear stable. Maintain the recommended buffer and monitor sell-through."
        st.markdown(f"<div class='decision'><h3>AI Decision</h3><p>{recommendation}</p></div>", unsafe_allow_html=True)

        if st.session_state.get("market_data"):
            st.markdown("<div class='card'><div class='eyebrow'>PRICE REFERENCES</div><h2 style='margin:0 0 8px'>Market results</h2><div class='small-note'>SerpAPI results supporting the market reference.</div></div>", unsafe_allow_html=True)
            rows = []
            for score, price, title, source, link in st.session_state.market_data.get("results", []):
                rows.append({"Product": title, "Price": f"₹{price:,.2f}", "Source": source})
            if rows:
                st.dataframe(pd.DataFrame(rows), use_container_width=True, hide_index=True)
    else:
        st.markdown("<div class='card'><div class='eyebrow'>READY</div><h2>Start a product analysis</h2><p class='small-note'>Enter your product and city. Refresh the market reference when needed, then generate the AI decision.</p></div>", unsafe_allow_html=True)

...
DEFAULTS = {
    ...
    "prediction": None, "forecast": None, "price": None, "price_source": None,
    "saved": []
}
for k, v in DEFAULTS.items():
    st.session_state.setdefault(k, v)

# Clear stale prediction objects created by older app versions.
def normalize_prediction(p):
    ...
    if not required.issubset(p.keys()):
        return None
    try:
        p["pred"] = float(p["pred"]); p["stock"] = float(p["stock"])
        p["temp"] = float(p["temp"]); p["holiday"] = int(p["holiday"])
        p["trend"] = int(p["trend"]); p["market"] = float(p["market"])
        p["suggested"] = float(p["suggested"]); p["confidence"] = int(p["confidence"])
...
def authenticate(email, password):
    ...
    return u if u and u.get("password") == ph(password) else None
...
def monthly_weather(city):
    try:
        ...
        if not f.empty:
            ...
            temps = {calendar.month_name[m]: round(float(g.get(m, 25)), 1) for m in range(1, 13)}
...
def get_model():
    ...
    model.fit(d[["holiday", "temp", "trend"]], d.sales)
    return model
...
def action_list(p):
    ...
    if p["suggested"] > p["market"] * 1.02:
        a.append(("warning", "Review pricing", "Demand-aware price is above the live market reference."))
    else:
        a.append(("info", "Keep price near market", "Limited price change is indicated."))
    return a[:4]
...
def inputs():
    ...
    with x:
        if st.button("Refresh live market price", use_container_width=True, key="price_button"):
            ...
            if price is None:
                ...
                st.session_state.price_source = msg
    ...
    if st.session_state.price is not None:
        st.markdown(f"<div style='background:#dcfce7;border-radius:12px;padding:14px 18px;color:#047857;font-weight:800'>Live market reference: ₹{st.session_state.price:,.2f}</div>", unsafe_allow_html=True)
...
def simulator():
    ...
    simulated_stock = int(np.ceil(simulated * 1.1))
    x, y = st.columns(2)
    ...
    with y: st.markdown(f"<div class='kpi'><small>RECOMMENDED STOCK</small><strong>{simulated_stock:,} units</strong><span>Includes 10% buffer</span></div>", unsafe_allow_html=True)


def dashboard():
    p = st.session_state.prediction
    a, b, c, d = st.columns(4)
    values = [("FORECAST DEMAND", f"{int(p['pred']):,} units" if p else "—", "Current month"), ("RECOMMENDED STOCK", f"{int(p['stock']):,} units" if p else "—", "10% planning buffer"), ("LIVE MARKET", f"₹{p['market']:,.2f}" if p else "—", "Required market reference"), ("CONFIDENCE", f"{p['confidence']}%" if p else "—", "Model confidence")]
    for col, (label, value, note) in zip([a, b, c, d], values):
    ...
    with b:
        ...
        for n, v in [("Product trend", f"{p['trend']}/100"), ("Holiday days", p["holiday"]), ("Temperature", f"{p['temp']:.1f} °C"), ("Confidence", f"{p['confidence']}%")]:
            st.markdown(f"<p class='small'><b>{n}</b> <span style='float:right'>{v}</span></p>", unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)
    simulator()
...
def product_page():
    ...
    if matches(p):
        market_text = f"₹{p['market']:,.2f}" if isinstance(p.get("market"), (int, float, np.number)) else "—"
        st.markdown(f"<div class='card'><div class='eyebrow'>LATEST RESULT</div><h3>{html.escape(str(p.get('product','')))} · {html.escape(str(p.get('city','')))} · {html.escape(str(p.get('month','')))}</h3><p class='small'>Forecast demand: <b>{int(p['pred']):,}</b> units · Weather: <b>{p['temp']:.1f} °C</b> · Holiday days: <b>{p['holiday']}</b> · Confidence: <b>{p['confidence']}%</b> · Market: <b>{market_text}</b> · Weather source: <b>{html.escape(str(p.get('source','')))}</b></p></div>", unsafe_allow_html=True)
...
def forecast_page():
    ...
    if not p or st.session_state.forecast is None:
        ...
        return
    t = st.session_state.forecast
    st.line_chart(t.set_index("Month")[["Temperature (°C)"]], height=250)
    st.line_chart(t.set_index("Month")[["Forecast demand"]], height=280)
    st.dataframe(t.assign(**{"Recommended stock": np.ceil(t["Forecast demand"] * 1.1).astype(int)}), use_container_width=True, hide_index=True)
...
def pricing_page():
    ...
    st.markdown(f"<div class='decision'><h3>Pricing rationale</h3><p>Recommendation uses the live market reference of ₹{p['market']:,.2f} and the current demand forecast of {int(p['pred']):,} units.</p></div>", unsafe_allow_html=True)
...
def compare_page():
    ...
    if st.button("Compare products →", use_container_width=True, key="compare_button"):
        ...
        for product in [p1, p2, p3]:
            if product.strip():
                t, city, _ = make_forecast(product, st.session_state.city)
                peak = t.loc[t["Forecast demand"].idxmax()]
                rows.append({"Product": product, "City": city, "12-month demand": int(t["Forecast demand"].sum()), "Peak month": peak.Month, "Peak demand": int(peak["Forecast demand"]), "Recommended stock": int(np.ceil(t["Forecast demand"].sum() * 1.1))})
...
def copilot_page():
    ...
    q = st.text_input("Ask a question", placeholder="Why should I increase stock?", key="question")
    if st.button("Analyze question →", use_container_width=True, key="ask"):
        ...
        if not p:
            ...
            if "stock" in query or "inventory" in query:
                ...
                answer = f"The live market reference is ₹{p['market']:,.2f}; the demand-aware recommendation is ₹{p['suggested']:,.2f}."
            else:
...
def settings_page():
    ...
    st.text_input("GST Number", value=u.get("gst", ""), disabled=True)
    st.text_input("Annual Turnover", value=u.get("turnover", ""), disabled=True)
    st.info("MongoDB and Brevo are intentionally disabled in this presentation build.")
...
if st.session_state.page == "welcome":
    header("OPTIRETAIL AI", "Turn market data into <span>smarter decisions.</span>", "Forecast demand, use live market intelligence, simulate scenarios, detect risks and explain AI recommendations.")
    a, b, c, d = st.columns(4)
    ...
    if submit:
        if not email or "@" not in email or not pw or not gst:
            st.error("Please enter a valid email, password and GST number.")
        else:
            ok, msg = signup(email, pw, gst, turnover)
            if ok:
    ...
    if st.session_state.page == "dashboard":
        header("DASHBOARD", "Turn market data into <span>smarter decisions.</span>", "AI Action Center, health score, explainable forecast and simulator."); dashboard()
        ...
        header("MARKET INSIGHTS", "Live market <span>intelligence.</span>", "Current market reference is required for the decision workflow.")
        price, msg = live_market_price(st.session_state.product, st.session_state.city)
        if price is None: st.error(msg)
        else: st.metric("Current market reference", f"₹{price:,.2f}"); st.caption(msg)
        ...
        for p in st.session_state.saved[:10]:
            p = normalize_prediction(p)
            if p:
